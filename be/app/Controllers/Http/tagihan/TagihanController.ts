...
export default class TagihanController {
    ...
    public async prosestagihan({ request, response }: HttpContextContract) {
        const trans_dtl_inv = await Database.connection("mssql_tagihan")                                
                        ...
                        .from(Database.raw("trans_details_inv with(nolock)"))                                
    ...
    public async tambahtagihansimpan ({ request, response }) {
        ...
        .rawQuery(" select case when a.sup_code is null then b.sup_code else a.sup_code end sup_code, a.sup_name from m_supplier a with(nolock) left join vend_data b with(nolock) on a.sup_code_app = b.supp_code_app where a.sup_npwp = ?", [request.input('empid')])

        var query = "select top 1 trans_code from (select top 1 a.trans_code, b.bu_alias as domain from trans_header a with(nolock), mst_businessunit b with(nolock) where a.bu_id = b.bu_id and b.bu_alias = '"+request.input('domain')+"' order by trans_code desc union all select top 1 trans_code,bu_domain as domain from "+Env.get('DBFINSYS')+".[dbo].[trans_header] with(nolock) where bu_domain = '"+request.input('domain')+"' order by trans_code desc) b order by trans_code desc";
        const trans_code = await Database.connection("mssql_finsys")
        .rawQuery(query)
        // .rawQuery("select top 1 trans_code from (select top 1 a.trans_code, b.bu_alias as domain from trans_header a with(nolock), mst_businessunit b with(nolock) where a.bu_id = b.bu_id and b.bu_alias = ? order by trans_code desc union all select top 1 trans_code,bu_domain as domain from "+Env.get('DBFINSYS')+".[dbo].[trans_header] with(nolock) where bu_domain = ? order by trans_code desc) b order by trans_code desc" , [request.input('domain'),request.input('domain')])

        const BU = await Database.connection("mssql_finsys")
        .rawQuery("select * from mst_businessunit with(nolock) where bu_alias = ?" , [request.input('domain')])
    ...
    public async tambahinvoicesimpan({ request, response }: HttpContextContract) {
        ...
        for (var i = 0; i < trans_dtl_po.length; i++ ){
            const dtl_po =  await Database.connection("mssql_tagihan")
                        ...
                        .from(Database.raw("trans_details_po with(nolock)"))                                
                        .select('*')
                        .where('trans_code', request.input('trans_code'))
    ...
    public async getListTagihan({request, response}) {                  
        ...
        const getBussinessUnit = await Database.connection("mssql_finsys")                                
                                .from(Database.raw("mst_businessunit with(nolock)"))                                
                                .where('bu_status', 1) 
                                .select('*') 
                                .select(
        ...
        const getStatusTagihan = await Database.connection("mssql_tagihan")                                
                                // .from('trans_header')          
                                .from(Database.raw("trans_header with(nolock)"))     
    ...
    public async getDetailInv({ request }) {  
        ...
        const sj = await Database.connection("mssql_tagihan")  
                    ...
                    .from(Database.raw("trans_details_sj with(nolock)"))     
                    .select('*') 
                    .select(
    ...
    public async getPO({ request }) {             
        ...
        .rawQuery("select * from m_supplier with(nolock) where sup_npwp = ?", [request.input('empid')])
        ...
        try {
            ...
            .rawQuery("select distinct dtl_po, dtl_no_mrr, dtl_site from trans_details_po a with(nolock), trans_header b with(nolock) where  a.trans_code = b.trans_code and b.bu_domain = ?  and b.vendor_code = ? "+ where +"", [request.input('domain'), res[0].sup_code])
    ...
    public async getPOMrr({ request }) {      
        ...
        try {
            ...
            const po_ereg = await Database.connection("mssql_tagihan")
                .rawQuery("select distinct dtl_no_mrr from trans_details_po a with(nolock), trans_header b with(nolock) where  a.trans_code = b.trans_code and b.bu_domain = ?  and b.vendor_code = ?", [request.input('domain'), res[0].sup_code])
            ...
            for (var i = 0; i < res_wsa.length; i++) {
                if (request.input('po_number') === res_wsa[i].nomorpo && (mrr_trans.includes(res_wsa[i].nomormrr) === false)) {
                    let ppn = (res_wsa[i].ppn === true) ? 'Ya' : 'Tidak'

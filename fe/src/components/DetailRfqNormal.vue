          ...
          <table v-else class="tw-w-full tw-text-sm  tw-text-gray-500  tw-mt-5">
            ...
            <tbody>
              <tr v-for="(data, i) in listData" :key="i"
                ...
                <td class="tw-py-2 tw-px-3 tw-text-right">
                  {{ formatRupiah(listData[i].harga_nego) }}
                </td>
                <td class="tw-py-2 tw-px-3">
                  {{ listData[i].status }}
                </td>
...
const save = async (value) => {
  ...
  $q.dialog({
    ...
    $q.loading.show()
    await axios.post(`${import.meta.env.VITE_API_ETENDER}saveRQbyVendor`, { code: props.data.rfqh_code, data: listData.value, vend: suppCode(), flag: value })
    $q.loading.hide()

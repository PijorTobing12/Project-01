      ...
      <q-card-section>
        ...
        <div class="tw-overflow-x-auto tw-relative">
          ...
          <div class="tw-mt-5">
            ...
            <button v-if="vendStatus != 2 && props.data.rfqs_flag == '1' && props.data.rfqh_flag != '4'" type="button"
...
import { domain, nik, suppCode, formatDate, formatRupiah } from '../utils';
...
const save = async (value) => {
  ...
  if (cekStatus > 0) {
    $q.notify({
      type: "negative",
      message: 'Terdapat data yang belum di set statusnya',
    });
    return;
  }
  ...
  $q.dialog({
    ...
    $q.loading.show()
    await axios.post(`${import.meta.env.VITE_API_ETENDER}saveRQbyVendorSplit`, { code: props.data.rfqh_code, data: listData.value, vend: suppCode(), flag: value })
    $q.loading.hide()

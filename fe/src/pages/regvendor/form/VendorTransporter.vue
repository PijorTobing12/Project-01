        ...
        <q-card class="my-card">
            ...
            <q-card-section class="q-mx-xs">
                <div class="q-mt-sm q-mb-sm q-mb-md">
                    <!-- <divider title="FORM KELENGKAPAN TAMBAHAN UNTUK VENDOR TRANSPORTERI" /> -->
                    <span class="text-indigo-10 tw-text-xl text-bold">FORM KELENGKAPAN TAMBAHAN UNTUK VENDOR TRANSPORTER</span>
                </div>
...
const getFile = async () => {
    try {
        $q.loading.show();
        const res = await axios.get("getVendorTransporter", {
            params: {
                ...pagination.value,
                code: route.params.code,
            },
...
const saveFile = async () => {
    try {
        ...
        if(tmpData.file == ''){
            ...
            const res = await axios.post("saveVendorTransporter", tmpData, {
                headers: {
...
const deleteFile = (value) => {
    $q.dialog({
        ...
        try {
            $q.loading.show();
            await axios.delete("delVendorTransporter/" + value.upload_id);
            $q.notify({
                type: "positive",

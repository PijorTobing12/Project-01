    ...
    <q-layout view="lHh Lpr lFf">
        <q-page-container>
            <q-page class="fit row wrap justify-center items-center content-center">
                <div class="q-pa-lg col-12 col-md-3 text-center login-page">
                    ...
                    <Form :initial-values="initialValues" :validation-schema="schema" @submit="onSubmit" class="q-mt-md">
                        ...
                        <Field v-model="tmpData.pass" name="password" v-slot="{ errorMessage, value, field }">
                            <q-input outlined dense :type="isPwd ? 'password' : 'text'" :model-value="value" label="Password" v-bind="field" 
                                :error-message="errorMessage" :error="!!errorMessage">
                    ...
                    <q-btn unelevated rounded color="secondary" class="full-width btn-login q-my-md" label="Register" @click="$router.replace('/register')" />
                    <q-btn unelevated rounded color="white" text-color="black" class="text-bold q-mt-sm full-width btn-login" label="Forgot Your Password?" @click="forgotPassword" />
                    <p class="text-grey-8 q-mt-sm">Copyright © 2023 IT DBC</p>
                </div>
            </q-page>
        </q-page-container>
...
const onSubmit = async (values, actions) => {
  try {
    $q.loading.show();
    const res = await axios.post("login", tmpData);
    actions.resetForm();
    window.localStorage.setItem("data", JSON.stringify(res.data));
    window.localStorage.setItem("token", res.data.token);
    window.localStorage.setItem("token1", res.data.token1);
    window.localStorage.setItem("token_tnd", res.data.token_tnd);
    window.localStorage.setItem("domain", res.data.data.domain);
    window.localStorage.setItem("nik", res.data.data.nik);
    window.localStorage.setItem("supp_code", res.data.vend);
    axios.defaults.headers.common["Authorization"] = 'Bearer ' + res.data.token;
    axios.defaults.headers.common["AuthorizationEtender"] = res.data.token1;
    axios.defaults.headers.common["AuthorizationVTND"] = res.data.token_tnd;

    /** Added by Zulvi */
    const nik = res.data.data.nik;
